...
agent_collect = DeepAgent(
  model         = model,
  system_prompt = create_prompt(
    personna     = personna,
    task         = args.task,
    focus        = Path( PROMPT_PATH, 'collect.md' ).read_text(),
...
agent_analyze = DeepAgent(
  model         = model,
  system_prompt = create_prompt(
    personna     = personna,
    task         = args.task,
    focus        = Path( PROMPT_PATH, 'analyze.md' ).read_text(),
    instructions = instructions
  ),
  backend       = backend,
  skills        = [str(SKILLS_PATH)],
...
agent_review = DeepAgent(
  model         = model,
  system_prompt = create_prompt(
    personna     = personna,
    task         = args.task,
    focus        = Path( PROMPT_PATH, 'review.md' ).read_text(),
    instructions = instructions
  ),
  backend       = backend,
  skills        = [str(SKILLS_PATH)],
...
agent_report = DeepAgent(
  model         = model,
  system_prompt = create_prompt(
    personna     = personna,
    task         = args.task,
    focus        = Path( PROMPT_PATH, 'report.md' ).read_text(),
    instructions = instructions
  ),
  backend       = backend,
  skills        = [str(SKILLS_PATH)],
...
chain = (
  ...
  | agent_collect.create_step( 'Collect', '1 - collect.md', isolate=args.isolate )
  | agent_analyze.create_step( 'Analyze', '2 - analyze.md', isolate=args.isolate )
  | agent_review.create_step(  'Review' , '3 - review.md', isolate=args.isolate )
  | agent_report.create_step(  'Report' , '4 - report.md', isolate=args.isolate )
)
